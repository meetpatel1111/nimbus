...
helm upgrade --install kube-prometheus prometheus-community/kube-prometheus-stack --namespace monitoring   --set prometheus.prometheusSpec.resources.requests.memory="256Mi"   --set prometheus.prometheusSpec.resources.requests.cpu="200m"   --set grafana.enabled=true   --set grafana.resources.requests.memory="256Mi"   --set grafana.resources.requests.cpu="200m"   --wait --timeout=${HELM_WAIT_TIMEOUT}

helm upgrade --install loki grafana/loki-stack --namespace monitoring   --set promtail.enabled=true   --set promtail.resources.requests.memory="64Mi"   --set promtail.resources.requests.cpu="50m"   --wait --timeout=${HELM_WAIT_TIMEOUT}
...
helm upgrade --install gitea gitea-charts/gitea --namespace ci   --set persistence.size=${GITEA_SIZE}   --set resources.requests.memory="256Mi"   --set resources.requests.cpu="200m"   --set admin.user=${GITEA_ADMIN_USER}   --set admin.password=${GITEA_ADMIN_PASS}   --wait --timeout=${HELM_WAIT_TIMEOUT}

helm upgrade --install drone drone/drone --namespace ci   --set env.DRONE_GITEA_SERVER=http://gitea.ci.svc.cluster.local   --set env.DRONE_RPC_SECRET=${DRONE_RPC_SECRET}   --set resources.requests.memory="128Mi"   --set resources.requests.cpu="100m"   --wait --timeout=${HELM_WAIT_TIMEOUT}
